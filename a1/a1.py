...
def bfs(graph, root, max_depth):
    ...
    while len(q) > 0:
        ...
        for nn in graph.neighbors(n):
          if(node2distances.get(n)+1<=max_depth):
            if nn not in visited:
              q.append(nn)
              visited.add(nn)
              node2distances[nn]=node2distances.get(n)+1
              node2num_paths[nn]=1
              node2parents[nn]=[n]
            else:
              if(node2distances.get(nn)>=(node2distances.get(n)+1)):
                if(node2distances.get(nn)==(node2distances.get(n)+1)):
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    b=deque()
    visited=set()
    max_value=max(node2distances.values())
    level=max_value
    ...
    while len(b) > 0:
      if(node2distances[b[0]]==level):
        n=b.popleft()
        if(n!=root):
          parents=node2parents.get(n)
          for y in parents:
            if y not in visited:
              visited.add(y)
              b.append(y)
            if(len(node2parents.get(n))>1):
              if(n<y):
                result[(n,y)]=float(node_credit.get(n)/len(node2parents.get(n)))
              else:
                result[(y,n)]=float(node_credit.get(n)/len(node2parents.get(n)))
            else:
              if(n<y):
                result[(n,y)]=float(node_credit.get(n))
              else:
                result[(y,n)]=float(node_credit.get(n))
            if(n<y):
              node_credit[y]=node_credit.get(y)+result.get((n,y))
            else:
              node_credit[y]=node_credit.get(y)+result.get((y,n))
      else:
        level=level-1
        for k,v in node2distances.items():
          if(v==level and (k not in visited)):
            b.append(k)
...
def approximate_betweenness(graph, max_depth):
    ...
    for x in graph.nodes():
      ...
      result=bottom_up(x,node2distances, node2num_paths, node2parents)
      for key,value in result.items():
        if key not in betweenness.keys():
          betweenness[key]=value
        else:
          betweenness[key]=betweenness.get(key)+value

    for key in betweenness.keys():
      betweenness[key]=betweenness.get(key)/2
...
def partition_girvan_newman(graph, max_depth):
    ...
    ###TODOz
    c_graph=graph.copy()
    betweenness=approximate_betweenness(c_graph,max_depth)
    s_betweenness=sorted(betweenness.items(),key=lambda x:(-x[1],x[0]))
    components = [c for c in nx.connected_component_subgraphs(c_graph)]
    for i in range(len(s_betweenness)):  
        if(len(components) == 1):
          c_graph.remove_edge(*s_betweenness[i][0])
          components = [c for c in nx.connected_component_subgraphs(c_graph)]
        else:
          break
    result = [c for c in sorted(components, key=lambda x: sorted(x.nodes())[0])]
    return result
    pass
...
def norm_cut(S, T, graph):
    ...
    """
    ###TODO
    cp=float(cut(S,T,graph))
    normalized_cut=float(cp/volume(S,graph))+float(cp/volume(T,graph))
...
def score_max_depths(graph, max_depths):
    ...
    scores=[]
    for i in max_depths:
      partitions=partition_girvan_newman(graph,i)
      scores.append((i,norm_cut(partitions[0].nodes(),partitions[1].nodes(),graph)))
    return scores
    pass
...
def make_training_graph(graph, test_node, n):
    ...
    ct_graph=graph.copy()
    neighbors=[]
    for nb in ct_graph.neighbors(test_node):
      neighbors.append(nb)
    neighbors=sorted(neighbors)
    i=0
    while(i<n):
      ct_graph.remove_edge(test_node,neighbors[i])
      i=i+1
    return ct_graph
    pass
...
def path_score(graph, root, k, beta):
    ...
    scores = []
    for n in sorted(graph.nodes()):
      if(n not in neighbors and n != root):
        distance=node2distances[n]
        paths=node2num_paths[n]
        scores.append(((root,n), (beta**distance)*paths))
    scores=sorted(scores, key=lambda x: x[1], reverse=True)
    return scores[:k]
    pass
...
def evaluate(predicted_edges, graph):
    ...
    for i in predicted_edges:
      if(graph.has_edge(i[0],i[1])):
        count=count+1
    accuracy=count/len(predicted_edges)
    return accuracy
    pass
