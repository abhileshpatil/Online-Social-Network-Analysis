...
def plot_sorted_accuracies(results):
    ...
    sorted_all_accuracies=sorted(all_accuracies)
    plt.plot(range(42), sorted_all_accuracies,'bo-')
    plt.xlabel('setting')
    plt.ylabel('accuracy')
...
def mean_accuracy_per_setting(results):
    ...
    """
    ###TODO
    all_accuracy_setting=[]
    accuracy_setting={}
    all_setting=[]
    z=0
    total_accuracy=defaultdict(lambda: 0)
    total_count=defaultdict(lambda: 0)
    head=defaultdict(list)
    for res in results:
      if res['features'] not in total_accuracy:
        head[res['features']]= 'features='+' '.join([func.__name__ for func in list(res['features'])])
        total_accuracy[res['features']]=res['accuracy']
        total_count[res['features']]=1
      else:
        total_accuracy[res['features']]=total_accuracy[res['features']]+res['accuracy']
        total_count[res['features']]=total_count[res['features']]+1
      if res['punct'] not in total_accuracy:
        head[res['punct']]= "punct="+str(res['punct'])
        total_accuracy[res['punct']]=res['accuracy']
        total_count[res['punct']]=1
      else:
        total_accuracy[res['punct']]=total_accuracy[res['punct']]+res['accuracy']
        total_count[res['punct']]=total_count[res['punct']]+1
      if res['min_freq'] not in total_accuracy:
        head[res['min_freq']]= "min_freq="+str(res['min_freq'])
        total_accuracy[res['min_freq']]=res['accuracy']
        total_count[res['min_freq']]=1
      else:
        total_accuracy[res['min_freq']]=total_accuracy[res['min_freq']]+res['accuracy']
        total_count[res['min_freq']]=total_count[res['min_freq']]+1

    for k,v in head.items():
      mean=total_accuracy[k]/total_count[k]
      tup=tuple((mean,v))
      all_accuracy_setting.append(tup)
    return(sorted(all_accuracy_setting, key=lambda x: (-x[0])))
pass
...
def top_coefs(clf, label, n, vocab):
    ...
    if(label==1):
      ...
      pos_negcoef=[x for x in zip(top_coef_terms, top_coef)]
    else:
      top_coef_ind = np.argsort(coef)[:n]
      top_coef_terms = np.array([k for k,v in sorted(vocab.items(), key=lambda x: x[1])])[top_coef_ind]
      top_coef = coef[top_coef_ind]
      pos_negcoef=[x for x in zip(top_coef_terms, top_coef*-1)]
